...
float getEdge() {
  ...
  // first column
  float tx0y0 = texture2D( tMap, vUv + texel * vec2( -1, -1 ) ).r;
  float tx0y1 = texture2D( tMap, vUv + texel * vec2( -1,  0 ) ).r;
  float tx0y2 = texture2D( tMap, vUv + texel * vec2( -1,  1 ) ).r;
  // second column
  float tx1y0 = texture2D( tMap, vUv + texel * vec2(  0, -1 ) ).r;
  float tx1y1 = texture2D( tMap, vUv + texel * vec2(  0,  0 ) ).r;
  float tx1y2 = texture2D( tMap, vUv + texel * vec2(  0,  1 ) ).r;
  // third column
  float tx2y0 = texture2D( tMap, vUv + texel * vec2(  1, -1 ) ).r;
  float tx2y1 = texture2D( tMap, vUv + texel * vec2(  1,  0 ) ).r;
  float tx2y2 = texture2D( tMap, vUv + texel * vec2(  1,  1 ) ).r;
